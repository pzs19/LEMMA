...
def load_prompt(data_name, prompt_type, num_shots):
    ...
    if "cot-meta-math" in prompt_type and data_name in ["gsm8k"]:
        data_name = "gsm8k-4shot"
    if data_name not in EXAMPLES:
        return []
    n = num_shots if num_shots <= len(EXAMPLES[data_name]) else len(EXAMPLES[data_name])
    return EXAMPLES[data_name][:n]
