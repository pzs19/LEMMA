...
def test_vanilla_checkpointing(disable_gradient_checkpointing: bool):
    model = load_train_model(
        disable_gradient_checkpointing=disable_gradient_checkpointing, **TRAIN_ARGS
    )
    for module in filter(
        lambda m: hasattr(m, "gradient_checkpointing"), model.modules()
    ):
        assert (
            getattr(module, "gradient_checkpointing") != disable_gradient_checkpointing
        )
...
def test_unsloth_gradient_checkpointing():
    ...
    for module in filter(
        ...
        assert (
            module._gradient_checkpointing_func.__self__.__name__
