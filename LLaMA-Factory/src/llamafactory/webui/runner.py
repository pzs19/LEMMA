...
class Runner:
    ...
    def monitor(self):
        ...
        if self.do_train:
            if (
                os.path.exists(os.path.join(output_path, TRAINING_ARGS_NAME))
                or use_ray()
            ):
            ...
            if (
                os.path.exists(os.path.join(output_path, "all_results.json"))
                or use_ray()
            ):
                finish_info = load_eval_results(
